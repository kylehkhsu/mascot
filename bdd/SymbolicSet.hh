...
public:
    ...
    // kyle
    /* var: idBddVars_
                * integer array[nvars_] containing the indexes of the bdd variables;
                * should be equivalent to indBddVars_, just in a more convenient format
            * for addMinterm, etc. */
    size_t* idBddVars_;
    double tau_;
    // end
    ...
    /* constructor: SymbolicSet
            ...
            */
    SymbolicSet(Cudd &ddmgr, const size_t dim, const double* lb, const double* ub, const double* eta, const double tau, const double* z=NULL) {
...
//            firstGridPoint_[i]=Nl*eta[i];
        ...
        tau_ = tau;
        size_t k = 0;
        idBddVars_ = new size_t[nvars_];
        for (size_t i = 0; i < dim_; i++) {
            for (size_t j = 0; j < nofBddVars_[i]; j++) {
                idBddVars_[k] = indBddVars_[i][j];
                k++;
            }
        }
        // end
    ...
    SymbolicSet(const SymbolicSet& other, std::vector<size_t> pdim) {
        ...
        idBddVars_ = new size_t[nvars_];
        for (size_t i = 0; i < dim_; i++) {
            for (size_t j = 0; i < nofBddVars_[i]; j++) {
                idBddVars_[k] = indBddVars_[i][j];
                k++;
            }
        }
...
//            z_[set1.dim_+i] = set1.z_[i];
    ...
    /* function: getTau
     */
    inline double getTau(void) const {
        return tau_;
    }
    ...
    /* function: addByFunction
            ...
            */
    template<class F>
    int addByFunction(F &function) {
        int iter = 0;

        /* set up grid to iterate over */
        BDD grid= !(symbolicSet_);
        /* set to be added */
...
//                std::cout << '\n';
    ...
    void writeToFile(const char *filename) {
        ...
        tau << "#scots time parameter tau: " << tau_ << std::endl;
...
//                std::cout << firstGridPoint_[i]+num*eta_[i] << '\n';
    ...
    /* function: isMinterm
            * minterm should have length of nvars_, don't cares don't work */
    bool isMinterm(int* minterm) {
        BDD b = mintermToBDD(minterm);
        if ((b & symbolicSet_) != ddmgr_->bddZero()) {
            return true;
        }
        else {
            return false;
    ...
    /* function: addMinterm
            * adds the minterm to the symbolic set BDD (2 = don't care) */
    ...
    /* function: mintermToBDD
                * computes and returns the BDD representing the minterm
                * minterm must be only for SymbolicSet's domain (not whole ddmgr)
                * note: for each dimension, the binary representation of the grid point index
                * is reversed. additionally, use 2 in the minterm to specify don't care for the dimension
            */
    BDD mintermToBDD(int* minterm) {
        BDD cube = ddmgr_->bddOne();
        size_t k = 0;
        for (size_t i = 0; i < dim_; i++) {
            size_t nofBddVars = nofBddVars_[i];
            if (minterm[k] == 2) {
                k += nofBddVars;
                continue;
            }
            ...
            std::copy(minterm + k, minterm + k + nofBddVars, phase + 0);
            k += nofBddVars;
            BDD* vars = new BDD[nofBddVars];
            for (size_t j = 0; j < nofBddVars; j++) {
                vars[j] = ddmgr_->bddVar(indBddVars_[i][j]);
            }
            cube &= ddmgr_->bddComputeCube(vars, phase, nofBddVars);
            delete[] phase;
            delete[] vars;
        }
        return cube;
    ...
    /* function pointToMinterm
                * computes the minterm corresponding to the nearest grid point
                * NAN value of dimension i => literals = 2 for dimension i
            */

    void pointToMinterm(double* point, int* minterm) {
        int* p = minterm;
        for (size_t i = 0; i < dim_; i++) {
...
//                std::cout << "idx: " << idx << '\n';
    ...
    /* function:  getNVars
                *
            * get the total number of bdd variables */
    inline size_t getNVars(void) const {
        return nvars_;
    }
    ...
    //    BDD mintermToBDD(int* minterm) {
    //        BDD cube = ddmgr_->bddOne(); // to trim down
    //        size_t k = 0; // starting index of the ith dimension in idBddVars_
    //        for (size_t i = 0; i < dim_; i++) { // for each dimension
    //            size_t nofBddVars = nofBddVars_[i]; // save the number of bdd vars
    //            std::vector<size_t> idx; // indexes of iddBddVar_[i] that we want

    //            for (size_t j = k; j < k + nofBddVars_[i]; j++) { // for each literal in the minterm in this dimension
    //                if (minterm[j] == 2) { // if the literal is don't care
    //                    nofBddVars -= 1; // update the number of bdd vars we will use
    //                    continue; // skip adding index
    //                } else {
    //                    idx.push_back(j - k); // add index to
    //                }
    //            }
    ...
    //                vars[j] = ddmgr_->bddVar(indBddVars_[i][*it]);
    //                phase[j] = minterm[j + k];
    //                j++;
    //            }
    //            cube &= ddmgr_->bddComputeCube(vars, phase, nofBddVars);
