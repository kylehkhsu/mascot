...
public:
   ...
   *
   */
  BDD pre(BDD Z, int debug = 0)  {
    /* project onto state alphabet */
    Z=Z.ExistAbstract(cubePost_*cubeInput_);

    if (debug) {
        SymbolicSet Zss(*symbolicModel_->stateSpace_);
        Zss.symbolicSet_ = Z;
        std::cout << "Z = Z.ExistAbstract(cubePost_*cubeInput_)\n";
        Zss.printInfo(1);
    }
    ...
    if (debug) {
        SymbolicSet nZss(*symbolicModel_->stateSpacePost_);
    ...
    if (debug) {
        SymbolicSet Fss(*symbolicModel_->stateSpace_, *symbolicModel_->inputSpace_);
        Fss.symbolicSet_ = F;
        std::cout << "BDD F = R_.AndAbstract(nZ,cubePost_)\n";
        Fss.printInfo(1);
    }
    ...
    if (debug) {
        SymbolicSet nFss(*symbolicModel_->stateSpace_, *symbolicModel_->inputSpace_);
    ...
    if (debug) {
        SymbolicSet preZss(*symbolicModel_->stateSpace_, *symbolicModel_->inputSpace_);
        preZss.symbolicSet_ = preZ;
        std::cout << "BDD preZ= RR_.AndAbstract(nF,cubePost_)";
        preZss.printInfo(1);
    }
   ...
   *
   */
  BDD reach(const BDD &T, const BDD &I, int earlyBreak, int verbose=0, int debug = 0)  {
    ...
    for(i=1; ZZ != Z; i++ ) {
      Z=ZZ;
      ZZ=FixedPoint::pre(Z, i == 0) | T;
      /* new (state/input) pairs */
      BDD N = ZZ & (!(C.ExistAbstract(cubeInput_)));

      if (debug || i == 0) {

          SymbolicSet Nss(*symbolicModel_->stateSpace_, *symbolicModel_->inputSpace_);
          Nss.symbolicSet_ = N;
          std::cout << "BDD N = ZZ & (!(C.ExistAbstract(cubeInput_)))";
          Nss.printInfo(1);
      }
