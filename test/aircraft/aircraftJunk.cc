...
const double w[dimX] = {0, 0, 0};

/* system parameters */
const double m = 60*1000;
const double g = 9.81;
...
auto sysNext = [](X_type &x, U_type &u, double tau, OdeSolver solver) -> void {
  ...
  auto rhs =[](X_type& xx,  const X_type &x, U_type &u) -> void {
      double D = (2.7 + 3.08 * pow((1.25 + 4.2*u[1]),2))*pow(x[0],2);
      double L = (68.6 * (1.25 + + 4.2*u[1]))*pow(x[0],2);
      xx[0] = (1/m) * (u[0]*cos(u[1]) - D - m*g*sin(x[1]));
      xx[1] = (1/(m*x[0])) * (u[0]*sin(u[1]) + L - m*g*cos(x[1]));
      xx[2] = x[0]*sin(x[1]);
  };
  solver(rhs,x,u);
};
