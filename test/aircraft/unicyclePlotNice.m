...
function unicyclePlotNice (mode, numAbs, controllers)
  w = [0.05 0.05 0];
  addpath(genpath('../..'));
...
%      plotCells(G,'facecolor',colors(2,:)*0.5+0.5,'edgec',colors(2,:),'linew',.1)
    savefig('simulation');
    print -depsc unicyclePlot.eps
  end

    if strcmp(mode,'plotReverse')
		green = [0.7569    0.8667    0.7765];
		purple = [0.8196    0.6549    0.8471];
		orange = [0.9137    0.8275    0.3804];
		red = [0.9373    0.2980    0.2980];
    	...
    	% load and draw goal
         ...
         % load and draw controller domains
            for i = 1:controllers
                for j=i:-1:1
                    Z = SymbolicSet(['Z/Z' int2str(j) '.bdd'], 'projection', [1 2]);
                    if Z.eta(1) == 0.6
                        thiscolor = green;
                    elseif Z.eta(1) == 0.3
                        thiscolor = purple;
