...
auto prod0AddG = [](SymbolicSet* G) -> void {
    auto f = [](double* ba0x)->bool {
        return sqrt(pow(ba0x[0]-0, 2) + pow(ba0x[1]-ba0x[2], 2)) <= 1;
    };
    G->addByFunction(f);
};
...
auto prod1AddG = [](SymbolicSet* G) -> void {
    auto f = [](double* ba1x)->bool {
        return sqrt(pow(ba1x[0]-0, 2) + pow(ba1x[1]-ba1x[2], 2)) <= 1;
    };
    G->addByFunction(f);
};
...
void composition() {
    ...
    double a0LbX[a0DimX] = {    2, -1.5};
    double a0UbX[a0DimX] = {    5,  1.5};
    double a0EtaX[a0DimX] = { 0.4,  0.4};
    a0X_t a0x;
    double a0EtaRatioX[a0DimX] = {2, 2};

    double a1LbX[a1DimX] = {  -5, -1.5};
    double a1UbX[a1DimX] = {  -2,  1.5};
