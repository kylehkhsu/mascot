...
auto radNext = [](X_type &r, U_type &u, double tau, OdeSolver solver) -> void {
    auto radODE = [](X_type &drdt, const X_type &r, const U_type &u) -> void {
        ...
        drdt[0] = L[0][0]*r[0] + L[0][1]*r[1];
        drdt[1] = L[1][0]*r[0] + L[1][0]*r[1];
    };
    solver(radODE, r, u);
};
...
//    double c[2] = {9, 9};
//    auto f = [c](double* x)->bool {
////        cout << x[0] << ' ' << x[1] << ' ' << sqrt(pow(x[0]-9, 2) + pow(x[1]-9, 2)) << '\n';
//        return sqrt(pow(x[0]-9, 2) + pow(x[1]-9, 2)) <= 1;
//    };
