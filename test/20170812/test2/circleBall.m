...
function circleBall (mode, controllers)
  ...
  if (mode == 'Q') % always eventually SCOTS
    ...
    while(1)
      ...
      for i = 1:-1:0
	if (i == 0)
	  C = SymbolicSet('goalCSCOTS.bdd', 'projection', [1 2 3 4 5]);
	  G = SymbolicSet('ESCOTS.bdd');
	else
	  C = SymbolicSet('CSCOTS.bdd', 'projection', [1 2 3 4 5]);
	  G = SymbolicSet('G.bdd');
	end
	...
	if (i == 1)
	  B = SymbolicSet('G.bdd','projection',[1 2]);
