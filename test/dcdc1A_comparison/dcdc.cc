...
//    double etaX[dimX]= {2/4e3*2*2, 2/4e3*2*2};
    double etaX[dimX]= {0.00200005/2, 0.00200005/2};
    double tau = 0.5/2;
    ...
    AdaptAbsSafe abs("dcdc1A_Lazy.log");
    ...
    Safe safe("dcdc1A_HSCC.log");
    safe.initialize(&dcdc, readAbs, dcdcAddO);
    safe.initializeSafe(dcdcAddS);
    safe.computeAbstractions(sysNext, radNext, x, u);
    safe.safe();

    // std::freopen("dcdc1A_SCOTS.log", "w", stderr);
    // TicToc tt;
    // Cudd mgr;
    // scots::SymbolicSet X(mgr,dimX,lbX,ubX,etaX,tau);
    // X.addGridPoints();
    // scots::SymbolicSet is(mgr,dimU,lbU,ubU,etaU, 0);
    // U.addGridPoints();
    // scots::SymbolicSet X2(X,1);
    // scots::SymbolicModelGrowthBound<X_type, U_type> abstraction(&X, &U, &X2);

    // tt.tic();
    // abstraction.computeTransitionRelation(sysNext, radNext, );
    // std::clog << "abstraction computation: " << tt.toc() << " seconds.\n";

    // scots::SymbolicSet safe(X);
    // dcdcAddS(&safe);
    ...
    // tt.tic();
    // C = fp.safe(Z);
    // std::clog << "controller synthesis: " << tt.toc() << " seconds.\n";
