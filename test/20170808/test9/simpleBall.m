function simpleBall (mode, controllers)
...
%      drawnow
%      I = SymbolicSet('I.bdd', 'projection', [3 4]);
%      plotCells(I, 'facecolor', colors(2,:)*0.5+0.5, 'edgec', colors(3,:), 'linew', 0.1)
%      drawnow
  ...
  if (mode == 'C')
    ...
    for i = controllers:-1:1
      ...
      C = SymbolicSet(['C/C' int2str(i) '.bdd'], 'projection', [1 2 3 4]);
      if (i == 1)
	G = SymbolicSet(['G.bdd']);
      else
...
%        plot(points(:,1), points(:,2), 'x', 'MarkerFaceColor', colors(mod(i,7)+1,:)*0.3+0.3, 'MarkerSize', 1.5);
    ...
    G = SymbolicSet('G.bdd', 'projection', [3 4]);
    g = G.getInputs(x(end,3:4));
...
function r = radNext(eta, u, tau)
  r(1) = eta(1) / 2 + abs(u(1)) .* tau / 4;
  r(2) = eta(2) / 2 + abs(u(1)) .* tau / 4;
  r(3) = eta(3) / 2 / 1.001 * tau;
  r(4) = eta(4) / 2 / 1.001 * tau;
end
