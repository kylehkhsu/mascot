...
auto unicycleAddO = [](SymbolicSet* O) -> void {
    ...
    double obs9[4] = {-10.3, 10.7, -3.7, 4.1};
    O->addPolytope(4, H, obs9, OUTER);

    double obs10[4] = {-10.3, 10.7, -1.9, 2.3};
    O->addPolytope(4, H, obs10, OUTER);

    double obs11[4] = {-12.7, 13.1, -1.9, 2.3};
    O->addPolytope(4, H, obs11, OUTER);

    double obs12[4] = {-12.7, 13.1, -3.7, 4.1};
    O->addPolytope(4, H, obs12, OUTER);

    double obs13[4] = {-15.1, 15.5, -3.7, 4.1};
    O->addPolytope(4, H, obs13, OUTER);

    double obs14[4] = {-15.1, 15.5, -1.9, 2.3};
    O->addPolytope(4, H, obs14, OUTER);

    double obs15[4] = {-17.5, 17.9, -1.9, 2.3};
    O->addPolytope(4, H, obs15, OUTER);

    double obs16[4] = {-17.5, 17.9, -3.7, 4.1};
    O->addPolytope(4, H, obs16, OUTER);

    double obs17[4] = {-19.9, 20.3, -3.7, 4.1};
    O->addPolytope(4, H, obs17, OUTER);

    double obs18[4] = {-19.9, 20.3, -1.9, 2.3};
    O->addPolytope(4, H, obs18, OUTER);

    double obs19[4] = {-22.3, 22.7, -1.9, 2.3};
    O->addPolytope(4, H, obs19, OUTER);

    double obs20[4] = {-22.3, 22.7, -3.7, 4.1};
    O->addPolytope(4, H, obs20, OUTER);
};
...
int main() {
    ...
    TicToc timer;
    timer.tic();
    abs.eagerReach(p, readAbs, sysNext, radNext, x, u);
