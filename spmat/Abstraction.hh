...
public:
 ...
 //       /* by kaushik: profiling */
  ...
  /**
   * @brief partially computes the transition function
   *
   * @param[out] transition_function - the result of the computation merged with the existing transitions
   *
   * @param[in] states_to_explore - the states for which the transitions are to be computed
   *
   * @param[in] system_post - lambda expression of the form
   *                          \verbatim [] (state_type &x, const input_type &u) ->  void  \endverbatim
   *                          system_post(x,u) provides a numerical approximation of ODE
   ...
   **/
  template<class F1, class F2, class F3=decltype(params::avoid_abs)>
  void compute_gb_partial(TransitionFunction& transition_function,
